...
// --- НОВОЕ: Состояние для файла логотипа ---
const selectedFile = ref(null);
const imagePreviewUrl = ref('');
// -------------------------------------

const isChanged = computed(() => {
    if (!props.currentTeam) return false; // Если нет currentTeam, но форма открыта (например, ошибка)
    const detailsChanged = formData.name !== (props.currentTeam.name || '') || 
                           formData.sport_id !== (props.currentTeam.sport_id || '');
    return detailsChanged || !!selectedFile.value; // Изменения есть, если изменились детали ИЛИ выбран файл
});
...
const handleSaveChanges = async () => {
    ...
    if (Object.keys(detailsPayload).length > 0) {
        try {
            ...
            finalTeamData = detailsResponse.data.team || finalTeamData; // Обновляем данными после редактирования
    ...
    if (selectedFile.value) {
        ...
        try {
            ...
            // --- ИЗМЕНЕНИЕ ЗДЕСЬ: Явно указываем Content-Type как undefined, чтобы Axios сам его установил для FormData ---
            const logoResponse = await apiClient.post(
                `/teams/${props.currentTeam.id}/logo`, 
                logoFormData,
                {
                    headers: {
                        'Content-Type': undefined // Или 'multipart/form-data', но undefined часто лучше, чтобы Axios сам добавил boundary
                    }
                }
            );
            // ----------------------------------------------------------------------------------------------------------
            ...
            isLoading.value = false;
             if(!teamDataUpdated && !Object.keys(detailsPayload).length > 0) return; // Если только лого не удалось, и детали не менялись
        }
    }
    ...
    if (!errorMsg.value) {
        if (teamDataUpdated || selectedFile.value) {
             ...
             emit('teamUpdated', finalTeamData); // Отправляем актуальные данные команды
        } else {
