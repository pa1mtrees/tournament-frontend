<template>
   ...
   const generateInviteLink = async () => {
    ...
    try {
        ...
        console.log("Invite API Response:", response.data); // Добавим лог для отладки ответа

        // --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
        // Проверяем наличие поля invite_token в ответе
        if (response.data && response.data.invite_token) { 
            const inviteToken = response.data.invite_token; // Используем invite_token
        // --------------------------
            
            generatedLink.value = `${window.location.origin}/invites/${inviteToken}`;

        } else {
            // Выбрасываем ошибку, если поле invite_token не найдено
