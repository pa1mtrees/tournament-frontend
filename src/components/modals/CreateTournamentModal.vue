...
import { ElMessage } from 'element-plus'; // Для сообщений валидации файла
...
const selectedTournamentLogoFile = ref(null); // Для хранения файла лого
const logoPreviewUrl = ref(''); // Для URL предпросмотра лого
...
// Обработчик выбора файла логотипа турнира
...
const handleCreateTournament = async () => {
  ...
  try {
    ...
    // Бэкенд должен вернуть { "tournament": { ... } }
    ...
    // Шаг 2: Если выбран файл логотипа, загружаем его
    if (selectedTournamentLogoFile.value) {
        successMsg.value += 'Uploading logo...';
        const logoFormData = new FormData();
        logoFormData.append('logo', selectedTournamentLogoFile.value); // Бэкенд ожидает 'logo'
        ...
        try {
            ...
            // Обновляем данные турнира, если бэк вернул обновленный объект с logo_url
            ...
            // Не сбрасываем errorMsg, чтобы пользователь видел обе части сообщения, если нужно
        }
    }
