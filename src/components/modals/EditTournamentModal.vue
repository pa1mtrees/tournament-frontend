...
<script setup>
import { ref, reactive, watch, computed, nextTick } from 'vue'; // Добавили nextTick
...
const errorMsg = ref('');
const successMsg = ref(''); // Это сообщение мы хотим видеть
...
const populateForm = () => {
    // Сбрасываем сообщения только при заполнении формы (т.е. при открытии)
    errorMsg.value = ''; 
    successMsg.value = '';
...
const closeModal = () => {
  // Сбрасываем только то, что не сбрасывается при populateForm при следующем открытии
  selectedLogoFile.value = null; 
  logoPreviewUrl.value = '';
  // errorMsg и successMsg будут сброшены при следующем populateForm
  emit('close');
};
