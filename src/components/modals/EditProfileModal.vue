        ...
        <section v-if="isOpen" class="max-w-2xl p-6 mx-auto bg-[var(--color-primary)] rounded-[1vw] shadow-xl w-full overflow-hidden">
            ...
            <form @submit.prevent="handleUpdateProfile">
                ...
                <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="label-style" for="profileNickname">Nickname (Optional)</label>
                    ...
                    <div>
                        <label class="label-style" for="profilePasswordConfirmation">Confirm New Password</label>
...
import { ElMessage } from 'element-plus'; // Для сообщений об ошибках валидации
import { Plus, EditPen } from '@element-plus/icons-vue'; // Иконки
...
// Используем authStore.userAvatar для отображения текущего аватара,
// так как он уже содержит логику генерации, если avatarUrl пользователя null.
const currentAvatarForForm = computed(() => {
    if (props.currentUser?.logo_url) { // Используем logo_url, как в вашем JSON
...
// Обработчик изменения файла в el-upload
const handleElFileChange = (uploadFile) => {
  // uploadFile.raw это объект File
  // beforeAvatarUpload уже должен был отработать
  if (uploadFile.raw && beforeAvatarUpload(uploadFile.raw)) { // Дополнительно проверяем, если beforeUpload не остановил
    selectedAvatarFile.value = uploadFile.raw;
    avatarPreviewUrl.value = URL.createObjectURL(uploadFile.raw);
    errorMsg.value = ''; // Сбрасываем предыдущие ошибки выбора файла
  } else {
    // Если файл не прошел валидацию в beforeAvatarUpload, el-upload его не добавит
    // Если мы хотим очистить превью, если пользователь выбрал невалидный файл,
    // а потом отменил выбор, то это сложнее, т.к. on-change может не вызваться для отмены.
    // Но если beforeAvatarUpload вернул false, файл не должен попасть в selectedAvatarFile.
    // Если пользователь выбрал файл, а потом убрал его из el-upload (если это возможно),
    // то selectedAvatarFile и avatarPreviewUrl нужно будет сбросить.
    // Для простоты, если uploadFile.raw нет, сбрасываем.
    if (!uploadFile.raw && selectedAvatarFile.value) { // Если файл был удален из списка el-upload
        selectedAvatarFile.value = null;
        avatarPreviewUrl.value = '';
    }
  }
...
const handleUpdateProfile = async () => {
    ...
    if (formData.email !== (props.currentUser?.email || '')) detailsPayload.email = formData.email; // Предполагаем, что email можно менять
    ...
    // 2. Загружаем аватар, если выбран новый файл
    if (selectedAvatarFile.value) {
        const avatarFormData = new FormData();
        // Бэкенд ожидает файл в поле 'avatar' (предположение, уточните у тиммейта)
...
<style scoped>
/* Стили для кастомизации el-upload */
.avatar-uploader-custom {
  width: 7rem; /* 112px, как w-28 */
  height: 7rem; /* 112px, как h-28 */
  display: flex;
  align-items: center;
  justify-content: center;
  position: absolute; /* Накладывается поверх фонового img */
  top: 0;
  left: 0;
  border-radius: 10%; /* Делаем круглым */
}
...
:deep(.avatar-uploader-custom .el-upload) {
  ...
  align-items: center;
  justify-content: center;
  background-color: rgba(0, 0, 0, 0.3); /* Полупрозрачный фон для видимости иконки/текста */
...
:deep(.avatar-uploader-custom .el-upload:hover) {
  ...
  background-color: rgba(0, 0, 0, 0.5);
    color: var(--color-myreddarker); /* Цвет иконки Plus/EditPen */
...
.avatar-preview {
  width: 100%;
  height: 100%;
  object-fit: cover; /* Предпросмотр нового аватара */
  border-radius: 10%;
  display: block;
}
