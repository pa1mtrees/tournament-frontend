    ...
    >
      <div class="flex-shrink-0">
        <img
          ...
          alt="User Avatar"
          class="w-24 h-24 rounded-full border-4 border-[var(--color-text-light)]"
        />
        <div
          v-else
          class="w-24 h-24 rounded-full bg-[var(--color-secondary)] border-4 border-[var(--color-myyellow)] flex items-center justify-center text-4xl text-[var(--color-text-light)] font-bold"
        >
      ...
      <div class="text-center sm:text-left flex-grow">
        ...
        <p v-if="user.created_at" class="text-xs text-[var(--color-text-muted)] mt-2">
    ...
    <div v-if="user" class="space-y-4">
      ...
      <div class="bg-[var(--color-primary)] p-6 rounded-[1vw] shadow-lg">
        <div v-if="user.team_id">
          <!-- {/* Отображение имени команды и логотипа */} -->
          <div
            class="flex items-center space-x-3 mb-4 pb-4 border-b border-[var(--color-secondary)]"
          >
            <!-- {/* Лого команды */} -->
            <img
              ...
              alt="Team Logo"
              class="w-16 h-16 rounded-md object-cover bg-[var(--color-secondary)]"
            />
            <div
              v-else
              class="w-16 h-16 rounded-md bg-[var(--color-secondary)] flex items-center justify-center text-[var(--color-text-muted)] text-xl"
            >
              T
            </div>

            <h4 class="text-2xl text-[var(--color-text-light)]">
              {{ user.team?.name || '...' }}
            </h4>
          </div>
            ...
            <thead class="bg-[var(--color-secondary)] text-left">
              <tr>
                ...
                <th
                  ...
                  class="p-3 text-[var(--color-text-light)] font-medium text-right"
                >
                  Actions
                </th>
              </tr>
                ...
                <td class="p-3">
                  <div class="flex items-center space-x-3">
                    <img
                      :src="
                        member.avatarUrl ||
                        `https://api.dicebear.com/8.x/identicon/svg?seed=${member.email || member.id}`
                      "
                      alt="Avatar"
