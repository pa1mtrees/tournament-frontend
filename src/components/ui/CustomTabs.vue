...
// tabs: массив объектов, описывающих каждый таб
//   - name: уникальный идентификатор таба (строка)
//   - label: текст для отображения на табе (строка)
//   - icon: (опционально) компонент Vue для иконки
//   - badgeContent: (опционально) содержимое для бейджа (число или строка)
// activeTab: имя текущего активного таба (строка)
...
// Определение событий, которые компонент может генерировать
const emit = defineEmits(['update:activeTab']);

// Функция для выбора таба и генерации события
...
<style scoped>
/* Для скрытия скроллбара, если табы не помещаются, но сохраняя возможность скролла */
